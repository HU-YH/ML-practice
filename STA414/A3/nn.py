...
def affine_backward(grad_y, x, w):
    ...
    #####################################################################
    #print('new iter')
    #print("affine grad_y:", grad_y.shape)
    #print("affine x:", x.shape)
    #print("affine w:", w.shape)
    grad_x = np.dot(grad_y,w.T)
    grad_w = np.dot(x.T,grad_y)
    grad_b = np.sum(grad_y,axis=0)
    #print('###########')
    #print("affine grad_x:", grad_x.shape)
    #print("affine grad_w:", grad_w.shape)
    #print("affine grad_b:", grad_b.shape)
    #print('###########')
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def relu_backward(grad_y, x):
    ...
    #####################################################################
    #print('relu')
    #print("relu grad_y:",grad_y.shape)
    #print("relu x:",x.shape)
    #print(grad_y)
    #print("...")
    #print(x)
    drelu=1. * (x > 0)
    #print("drelu:",drelu,'shape:',drelu.shape)
    grad_x = drelu*grad_y
    #print(grad_x.shape)
    #print("###################")
    #####################################################################
...
def train(model, forward, backward, update, eta, num_epochs, batch_size):
    ...
    train_misclassificaition_list=[]
    valid_misclassificaition_list=[]
    print(len(train_acc_list))
    for i in range(len(train_acc_list)):
        train_misclassificaition_list.append((i, 1-train_acc_list[i][1]))
        valid_misclassificaition_list.append((i, 1-valid_acc_list[i][1]))
        
    display_plot(train_misclassificaition_list, valid_misclassificaition_list, "Misclassification", number=2)
...
def last_question(inputs, target, model, forward, batch_size=-1,threshold=0.5):
    """ mainly the same as eval, minor changes
        """
    num_cases = inputs.shape[0]
    if batch_size == -1:
        batch_size = num_cases
...
def main():
    ...
    ##############################
    # for last question 2.5
    ##############################
    inputs_train, inputs_valid, inputs_test, target_train, target_valid, \
    target_test = load_data("toronto_face.npz")
    x = inputs_test
    t = target_test
    prediction = softmax(nn_forward(model, x)["y"])
    i=0
    #0.5 is the threshold
    for p in range(len(prediction)):
        if max(prediction[p])<0.5 & i<=5:
            #pseudo plot.image() function here, I don't know how to plot
            plot.image()
            print("Will the classifier be correct if it outputs the top scoring class anyways",np.argmax(prediction[p])==np.argmax(t[p]))
            i+=1
